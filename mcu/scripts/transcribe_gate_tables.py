...
def main():
    ...
    for src in ["nsl", "unsw"]:
        print(f"\n===== MCU gate: {src.upper()} (gamma={GATE['recheck'] and RECHECK['configs'][src]['coupled']['eps0.05_20'] and 'see gamma_params'}) =====")
        print(f"{'basis':<10s} {'eps':>4s} {'T':>4s} | {'naive':>7s} {'adapt':>7s} {'gap':>7s} | {'naive*':>7s} {'adapt*':>7s} {'gap*':>7s} {'gap/SE':>7s} | gate")
        print("-" * 100)
        for r in [x for x in matrix if x["source"] == src]:
            print(f"{r['basis']:<10s} {r['epsilon']:>4.2f} {r['T']:>4d} | "
                  ...
                  f"{r['n800']['naive_asr']:>7.3f} {r['n800']['adaptive_asr']:>7.3f} {r['n800']['gap']:>+7.3f} {r['n800']['gap_se']:>7.2f} | "
                  f"{'PASS' if r['n800']['gate'] else 'FAIL'}")
