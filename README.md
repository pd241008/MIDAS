...
- [x] MCU tier: gradient-to-query alignment for the UNSW non-confirmation — measured over the same 200-sample adaptive PGD batches as the gate (`mcu/scripts/grad_query_alignment.py`, `mcu/features/grad_query_alignment.json`). The natural geometric explanation is **refuted**: UNSW's loss-gradient energy inside the coupled rotation subspace span{v_t, v_t⊥} is 0.692 (mean |cos(∇J, v_t)| 0.664) vs NSL 0.279 (0.308), the opposite of what the mechanism predicts — the non-confirmation remains open but the first hypothesis is measured-out.
